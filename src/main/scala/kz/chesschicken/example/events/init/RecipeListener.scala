...
class RecipeListener {
  ...
  @EventListener def registerRecipes(event: RecipeRegisterEvent): Unit = {
    val crafting_shaped = RecipeRegisterEvent.Vanilla.CRAFTING_SHAPED.`type`.id
    val crafting_shapeless = RecipeRegisterEvent.Vanilla.CRAFTING_SHAPELESS.`type`.id
    val smelting = RecipeRegisterEvent.Vanilla.SMELTING.`type`.id


    event.recipeId.id match {
